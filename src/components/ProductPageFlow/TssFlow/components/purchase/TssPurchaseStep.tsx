 ...
 * TSS Purchase Step
 * 
 * TSS sigortası satın alma adımı
 * Kasko KaskoPurchaseStep ile birebir aynı yapıda
 */
...
import { pushTssPaymentSuccess, pushTssPaymentFailed } from '../../utils/dataLayerUtils';
...
export default function TssPurchaseStep({ onNext, onBack }: TssPurchaseStepProps) {
    ...
    useEffect(() => {
        const fetchQuoteDetails = async () => {
            try {
                ...
                const proposalId = proposalIdFromUrl || localStorage.getItem(TSS_STORAGE_KEYS.CURRENT_PROPOSAL) || localStorage.getItem(TSS_STORAGE_KEYS.PROPOSAL_ID);
                const productId = productIdFromUrl || localStorage.getItem(TSS_STORAGE_KEYS.SELECTED_PRODUCT);
                ...
                if (selectedProduct) {
                    ...
                    const storedQuote = localStorage.getItem(TSS_STORAGE_KEYS.SELECTED_QUOTE);
                    ...
                    localStorage.setItem(TSS_STORAGE_KEYS.SELECTED_QUOTE, JSON.stringify(quoteData));
                    localStorage.setItem(TSS_STORAGE_KEYS.CURRENT_PROPOSAL, proposalId);
                    localStorage.setItem(TSS_STORAGE_KEYS.SELECTED_PRODUCT, productId);
                } else {
                    const storedQuote = localStorage.getItem(TSS_STORAGE_KEYS.SELECTED_QUOTE);
                    if (storedQuote) {
            ...
            if (result2 || status === 'error' || error) {
                ...
                if (result2) {
                    try {
                        ...
                        if (parsed.success) {
                            localStorage.removeItem('paratika_3d_result');
                            router.push('/odeme/paratika-callback?type=tss&action=validate');
                            return;
                        }
                    } catch (e) { }
                }
    ...
    const cleanupLocalStorage = () => {
        ...
        localStorage.removeItem(TSS_STORAGE_KEYS.PROPOSAL_ID);
        localStorage.removeItem(TSS_STORAGE_KEYS.CURRENT_PROPOSAL);
        localStorage.removeItem(TSS_STORAGE_KEYS.SELECTED_QUOTE);
        localStorage.removeItem(TSS_STORAGE_KEYS.SELECTED_PRODUCT);
    };
    ...
    const handlePayment = async () => {
        ...
        let proposalIdFromStorage = localStorage.getItem(TSS_STORAGE_KEYS.PROPOSAL_ID);
        ...
        if (!proposalIdFromStorage) {
            try {
                const selectedQuoteForPurchase = localStorage.getItem(TSS_STORAGE_KEYS.SELECTED_QUOTE);
        ...
        try {
            ...
            const merchantPaymentId = `TSS-${proposalIdFromStorage}-${Date.now()}`;
                ...
                orderItems: [
                    {
                        ...
                        description: `${proposalIdFromStorage} numaralı TSS sigorta teklifi`,
                        quantity: 1,
                        amount: currentPremium.grossPremium,
                    },
                ],
    ...
    const proceedWithPurchase = async (threeDResult: any) => {
        try {
            ...
            const expiryParts = expiryDate.split('/');
            const proposalId = localStorage.getItem(TSS_STORAGE_KEYS.CURRENT_PROPOSAL);
            const customerProfile = await customerApi.getProfile();
            ...
            pushTssPaymentFailed(error.message);
        }
    };
    ...
    const handleViewOfferDocument = async () => {
        ...
        const proposalId = localStorage.getItem(TSS_STORAGE_KEYS.CURRENT_PROPOSAL) || selectedQuoteData.proposalId;
        const proposalProductId = selectedQuoteData.id;
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">Tamamlayıcı Sağlık Sigortası poliçeniz başarıyla oluşturuldu</span>
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">Tamamlayıcı Sağlık Sigortası satın alma işlemi tamamlanamadı.</span>
