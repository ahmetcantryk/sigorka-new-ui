 ...
 * ImmProductForm
 * 
 * Ürün detay sayfası için İMM formu
 * Trafik form yapısından adapte edildi
 */
...
import { ImmStepper, TramerErrorPopup } from './components/common';
...
import ImmProductQuote from './ImmProductQuote';
...
// Hooks
import { useImmVehicle } from './hooks/useImmVehicle';

// Utils
import { pushImmStep1Complete, pushImmStep2Complete } from './utils/dataLayerUtils';
...
// Types
import type { ImmFormProps, VehicleFormData } from './types';

const ImmProductForm = ({ onProposalCreated, onBack }: ImmFormProps) => {
  ...
  const formik = useFormik<VehicleFormData>({
    initialValues: IMM_FORM_DEFAULTS,
  ...
  const handlePurchaseClick = (quoteId: string) => {
    ...
    const selectedQuote = localStorage.getItem('selectedQuoteForPurchaseImm');
    if (selectedQuote) {
      const quoteData = JSON.parse(selectedQuote);
      localStorage.setItem('selectedQuoteForPurchaseImm', JSON.stringify({
        ...quoteData,
        id: quoteId
      }));
    }
  ...
  const handlePersonalInfoSubmit = async () => {
    ...
    if (formik.values.email?.trim()) {
      localStorage.setItem(IMM_STORAGE_KEYS.INITIAL_EMAIL, formik.values.email.trim());
      console.log('✅ Saved immInitialEmail:', formik.values.email.trim());
    }

    // Zaten login ise
    if (accessToken) {
      console.log('✅ Already authenticated, checking if email needs update');
      
      // E-posta güncellemesi gerekebilir
      const userEnteredEmail = formik.values.email?.trim() || localStorage.getItem(IMM_STORAGE_KEYS.INITIAL_EMAIL);
      ...
      if (userEnteredEmail) {
        try {
          ...
          if (meData && customerId) {
            // E-posta güncellemesi yap
            await updateUserProfileWithCurrentData(meData, userEnteredEmail, customerId);
          }
        } catch (error) {
  ...
  const handleVerifyCode = async (code: string) => {
    ...
    try {
      ...
      const userEnteredEmail = localStorage.getItem(IMM_STORAGE_KEYS.INITIAL_EMAIL);
      ...
      if (customerIdToUse) {
        ...
        localStorage.setItem(IMM_STORAGE_KEYS.PROPOSAL_ID, customerIdToUse);
      }

      // Update email
      if (customerIdToUse && userEnteredEmail) {
        try {
          await updateUserProfileWithCurrentData(meData, userEnteredEmail, customerIdToUse);
        } catch (error) {
          console.warn('Email update hatası:', error);
        }
      }
  ...
  const updateUserProfileWithCurrentData = async (
    ...
    try {
      ...
      // Mevcut job varsa koru
      if (currentMeData?.job) {
  ...
  const handleAdditionalInfoSubmit = async () => {
    ...
    try {
      ...
      // Mevcut job varsa koru
      if (currentMeData.job !== undefined && currentMeData.job !== null) {
        updatePayload.job = currentMeData.job;
      }
      ...
      localStorage.setItem(IMM_STORAGE_KEYS.PERSONAL_INFO_COMPLETED, 'true');
  ...
  const handleFormSubmit = async (values: VehicleFormData) => {
    ...
    try {
      if (selectionType === 'existing' && selectedVehicleId) {
        ...
        if (proposalId) {
          localStorage.setItem(IMM_STORAGE_KEYS.PROPOSAL_ID, proposalId);
  ...
  return (
    <>
      <div className="product-page-flow-container">
        <ImmStepper activeStep={activeStep} />
