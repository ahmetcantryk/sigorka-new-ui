 ...
 * IMM Purchase Step
 * 
 * İMM sigortası satın alma adımı
...
import { pushImmPaymentSuccess, pushImmPaymentFailed } from '../../utils/dataLayerUtils';
import { IMM_STORAGE_KEYS } from '../../config/immConstants';
...
interface ImmPurchaseStepProps {
    onNext?: () => void;
    onBack?: () => void;
}

export default function ImmPurchaseStep({ onNext, onBack }: ImmPurchaseStepProps) {
    ...
    useEffect(() => {
        const fetchQuoteDetails = async () => {
            try {
                ...
                // İMM'e özgü localStorage key'leri kullan
                const proposalId = proposalIdFromUrl || localStorage.getItem(IMM_STORAGE_KEYS.CURRENT_PROPOSAL) || localStorage.getItem(IMM_STORAGE_KEYS.PROPOSAL_ID);
                const productId = productIdFromUrl || localStorage.getItem(IMM_STORAGE_KEYS.SELECTED_PRODUCT);
                ...
                if (selectedProduct) {
                    // Önce localStorage'dan seçilen taksit bilgisini kontrol et (İMM key'i)
                    const storedQuote = localStorage.getItem(IMM_STORAGE_KEYS.SELECTED_QUOTE);
                    ...
                    localStorage.setItem(IMM_STORAGE_KEYS.SELECTED_QUOTE, JSON.stringify(quoteData));
                    localStorage.setItem(IMM_STORAGE_KEYS.CURRENT_PROPOSAL, proposalId);
                    localStorage.setItem(IMM_STORAGE_KEYS.SELECTED_PRODUCT, productId);
                } else {
                    const storedQuote = localStorage.getItem(IMM_STORAGE_KEYS.SELECTED_QUOTE);
                    if (storedQuote) {
            ...
            if (result2 || status === 'error' || error) {
                ...
                if (result2) {
                    try {
                        ...
                        if (parsed.success) {
                            localStorage.removeItem('paratika_3d_result');
                            router.push('/odeme/paratika-callback?type=imm&action=validate');
                            return;
                        }
                    } catch (e) { }
                }
    ...
    const cleanupLocalStorage = () => {
        ...
        localStorage.removeItem(IMM_STORAGE_KEYS.PROPOSAL_ID);
        localStorage.removeItem(IMM_STORAGE_KEYS.CURRENT_PROPOSAL);
    ...
    const handlePayment = async () => {
        ...
        let proposalIdFromStorage = localStorage.getItem(IMM_STORAGE_KEYS.PROPOSAL_ID);
        ...
        if (!proposalIdFromStorage) {
            try {
                const selectedQuoteForPurchase = localStorage.getItem(IMM_STORAGE_KEYS.SELECTED_QUOTE);
        ...
        try {
            ...
            const merchantPaymentId = `IMM-${proposalIdFromStorage}-${Date.now()}`;
                ...
                orderItems: [
                    {
                        ...
                        name: 'İMM Sigortası',
                        description: `${proposalIdFromStorage} numaralı İMM sigorta teklifi`,
                        quantity: 1,
                        amount: currentPremium.grossPremium,
                    },
                ],
    ...
    const proceedWithPurchase = async (threeDResult: any) => {
        try {
            ...
            const expiryParts = expiryDate.split('/');
            const proposalId = localStorage.getItem(IMM_STORAGE_KEYS.CURRENT_PROPOSAL);
            const customerProfile = await customerApi.getProfile();
            ...
            pushImmPaymentFailed(error.message);
        }
    };
    ...
    const handleViewOfferDocument = async () => {
        ...
        const proposalId = localStorage.getItem(IMM_STORAGE_KEYS.CURRENT_PROPOSAL) || selectedQuoteData.proposalId;
        const proposalProductId = selectedQuoteData.id;
        ...
        try {
            ...
            setErrorMessage('Teklif belgesi görüntülenirken bir hata oluştu.');
        }
    };
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">İMM Sigortası poliçeniz başarıyla oluşturuldu</span>
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">İMM Sigortası satın alma işlemi tamamlanamadı.</span>
