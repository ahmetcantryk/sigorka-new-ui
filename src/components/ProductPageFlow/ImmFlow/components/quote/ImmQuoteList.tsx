/**
 * İMM Flow - Quote List Bileşeni
 * 
 * İMM tekliflerini listeler (Trafik ile aynı yapı)
 */
...
const ImmQuoteList = ({
  ...
  const coverageGroups = getUniqueCoverageGroups(quotes as any);
  const filteredQuotes = filterQuotes(quotes as any, selectedFilter) as ProcessedImmQuote[];
  const sortedQuotes = sortQuotes(filteredQuotes as any, selectedSort) as ProcessedImmQuote[];
  ...
  return (
    <div className="pp-card">
      {/* Title */}
      <h2 className="pp-quote-title">İhtiyari Mali Mesuliyet (İMM) Sigortası Teklifleri</h2>

      {/* Top Controls */}
      <div className="pp-quote-controls" style={{justifyContent: 'flex-end'}}>
