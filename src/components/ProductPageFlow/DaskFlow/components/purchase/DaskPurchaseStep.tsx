 ...
 * DASK Purchase Step
 * 
 * DASK sigortası satın alma adımı
 * ProductPageFlow tasarımına uygun - Kasko/Trafik ile aynı yapı
 */
...
import { pushDaskPaymentSuccess, pushDaskPaymentFailed } from '../../utils/dataLayerUtils';
...
interface DaskPurchaseStepProps {
    onNext: () => void;
    onBack?: () => void;
}

export default function DaskPurchaseStep({ onNext, onBack }: DaskPurchaseStepProps) {
    ...
    useEffect(() => {
        const fetchQuoteDetails = async () => {
            try {
                ...
                const proposalId = proposalIdFromUrl || localStorage.getItem('currentProposalId') || localStorage.getItem('proposalIdForDask');
                const productId = productIdFromUrl || localStorage.getItem('selectedProductIdForDask');
                ...
                if (selectedProduct) {
                    ...
                    localStorage.setItem('selectedProductIdForDask', productId);
                } else {
                    const storedQuote = localStorage.getItem('selectedQuoteForPurchase');
                    if (storedQuote) {
            ...
            if (result2 || status === 'error' || error) {
                ...
                if (result2) {
                    try {
                        ...
                        if (parsed.success) {
                            localStorage.removeItem('paratika_3d_result');
                            router.push('/odeme/paratika-callback?type=dask&action=validate');
                            return;
                        }
                    } catch (e) { }
                }
    ...
    const handlePayment = async () => {
        ...
        let proposalIdFromStorage = localStorage.getItem('proposalIdForDask');
        ...
        try {
            ...
            const merchantPaymentId = `DASK-${proposalIdFromStorage}-${Date.now()}`;
                ...
                orderItems: [
                    {
                        ...
                        name: 'DASK Sigortası',
                        description: `${proposalIdFromStorage} numaralı DASK sigorta teklifi`,
                        quantity: 1,
                        amount: currentPremium.grossPremium,
                    },
                ],
            ...
            if (sessionResponse.responseCode !== '00') {
                ...
                pushDaskPaymentFailed('Session oluşturulamadı');
                return;
            }
            ...
            setShowErrorScreen(true);
            // Ödeme başarısız dataLayer push
            pushDaskPaymentFailed(error.message || 'Ödeme işlemi başlatılamadı');
        }
    };
    ...
    const proceedWithPurchase = async (threeDResult: any) => {
        try {
            ...
            // Ödeme başarısız dataLayer push
            pushDaskPaymentFailed(error.message);
        }
    };
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">DASK Sigortası poliçeniz başarıyla oluşturuldu</span>
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">DASK Sigortası satın alma işlemi tamamlanamadı.</span>
