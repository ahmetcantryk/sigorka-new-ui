/**
 * Trafik Purchase Step
 * 
 * Trafik sigortası satın alma adımı
...
interface TrafikPurchaseStepProps {
  onNext: () => void;
  onBack?: () => void;
}

export default function TrafikPurchaseStep({ onNext, onBack }: TrafikPurchaseStepProps) {
  ...
  useEffect(() => {
    const fetchQuoteDetails = async () => {
      try {
        ...
        const proposalId = proposalIdFromUrl || localStorage.getItem('currentProposalId') || localStorage.getItem('proposalIdForTrafik');
        const productId = productIdFromUrl || localStorage.getItem('selectedProductIdForTrafik');
        ...
        if (selectedProduct) {
          ...
          localStorage.setItem('selectedProductIdForTrafik', productId);
        } else {
      ...
      if (result2 || status === 'error' || error) {
        ...
        if (result2) {
          try {
            ...
            if (parsed.success) {
              localStorage.removeItem('paratika_3d_result');
              router.push('/odeme/paratika-callback?type=trafik&action=validate');
              return;
            }
          } catch (e) { }
        }
  ...
  const cleanupLocalStorage = () => {
    ...
    localStorage.removeItem('proposalIdForTrafik');
    localStorage.removeItem('currentProposalId');
    localStorage.removeItem('selectedQuoteInfo');
    localStorage.removeItem('trafikQuoteFlow');
  ...
  const handlePayment = async () => {
    ...
    let proposalIdFromStorage = localStorage.getItem('proposalIdForTrafik');
    ...
    try {
      ...
      const merchantPaymentId = `TRAFIK-${proposalIdFromStorage}-${Date.now()}`;
        ...
        orderItems: [
          {
            ...
            description: `${proposalIdFromStorage} numaralı trafik sigorta teklifi`,
            quantity: 1,
            amount: currentPremium.grossPremium,
          },
        ],
      ...
      <div className="product-page-form pp-form-wide">
        <div className="pp-card">
          <div className="pp-quote-error-container">
            ...
            <span className="pp-error-card-subtitle">Zorunlu Trafik Sigortası poliçeniz başarıyla oluşturuldu</span>
      ...
      <div className="product-page-form pp-form-wide">
        <div className="pp-card">
          <div className="pp-quote-error-container">
            ...
            <span className="pp-error-card-subtitle">Zorunlu Trafik Sigortası satın alma işlemi tamamlanamadı.</span>
