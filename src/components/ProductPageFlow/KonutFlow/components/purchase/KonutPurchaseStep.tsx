 ...
 * Konut Purchase Step
 * 
 * Konut sigortası satın alma adımı
 * ProductPageFlow tasarımına uygun - DASK ile aynı yapı
 */
...
import { pushKonutPaymentSuccess, pushKonutPaymentFailed } from '../../utils/dataLayerUtils';
...
interface KonutPurchaseStepProps {
    onNext: () => void;
    onBack?: () => void;
}

export default function KonutPurchaseStep({ onNext, onBack }: KonutPurchaseStepProps) {
    ...
    useEffect(() => {
        const fetchQuoteDetails = async () => {
            try {
                ...
                const proposalId = proposalIdFromUrl || localStorage.getItem('currentProposalIdKonut') || localStorage.getItem('proposalIdForKonut');
                const productId = productIdFromUrl || localStorage.getItem('selectedProductIdForKonut');
                ...
                if (response.status === 401) {
                    ...
                    window.location.href = '/konut-sigortasi';
                    return;
                }
                ...
                if (selectedProduct) {
                    ...
                    localStorage.setItem('currentProposalIdKonut', proposalId);
                    localStorage.setItem('selectedProductIdForKonut', productId);
                } else {
                    const storedQuote = localStorage.getItem('selectedQuoteForPurchase');
                    if (storedQuote) {
            ...
            if (result2 || status === 'error' || error) {
                ...
                if (result2) {
                    try {
                        ...
                        if (parsed.success) {
                            localStorage.removeItem('paratika_3d_result');
                            router.push('/odeme/paratika-callback?type=konut&action=validate');
                            return;
                        }
                    } catch (e) { }
                }
    ...
    const handlePayment = async () => {
        ...
        let proposalIdFromStorage = localStorage.getItem('proposalIdForKonut');
        ...
        try {
            ...
            if (sessionResponse.responseCode !== '00') {
                ...
                pushKonutPaymentFailed('Session oluşturulamadı');
                return;
            }
            ...
            setShowErrorScreen(true);
            // Ödeme başarısız dataLayer push
            pushKonutPaymentFailed(error.message || 'Ödeme işlemi başlatılamadı');
        }
    };
    ...
    const proceedWithPurchase = async (threeDResult: any) => {
        try {
            ...
            // Ödeme başarısız dataLayer push
            pushKonutPaymentFailed(error.message);
        }
    };
    ...
    const handleViewOfferDocument = async () => {
        ...
        const proposalId = localStorage.getItem('currentProposalIdKonut') || selectedQuoteData.proposalId;
        const proposalProductId = selectedQuoteData.id;
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">Konut Sigortası poliçeniz başarıyla oluşturuldu</span>
            ...
            <div className="product-page-form pp-form-wide">
                <div className="pp-card">
                    <div className="pp-quote-error-container">
                        ...
                        <span className="pp-error-card-subtitle">Konut Sigortası satın alma işlemi tamamlanamadı.</span>
