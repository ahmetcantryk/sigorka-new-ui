...
interface UseKonutPropertyResult {
    // Data
    properties: any[];
...
export const useKonutProperty = (): UseKonutPropertyResult => {
    ...
    useEffect(() => {
        const fetchCities = async () => {
            try {
                console.log('🔍 Konut - Cities API çağrısı yapılıyor:', API_ENDPOINTS.ADDRESS_CITIES);
                const response = await fetchWithAuth(API_ENDPOINTS.ADDRESS_CITIES);
                console.log('✅ Konut - Cities API response status:', response.status, response.ok);
                if (response.ok) {
                    ...
                    console.log('✅ Konut - Cities yüklendi:', sortedCities.length, 'şehir');
                } else {
                    const errorText = await response.text();
                    console.warn('⚠️ Konut - Cities API hatası:', response.status, errorText);
                }
            } catch (error) {
                console.error('❌ Konut - Failed to fetch cities:', error);
            }
        };
        fetchCities();
