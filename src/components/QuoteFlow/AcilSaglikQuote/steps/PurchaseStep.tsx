...
export default function PurchaseStep({ onNext }: PurchaseStepProps) {
      ...
      if (result2 || status === 'error' || error) {
        ...
        if (result2) {
          try {
            ...
            if (parsed.success) {
              localStorage.removeItem('paratika_3d_result');
              router.push('/odeme/paratika-callback?type=Saglik&action=validate');
              return;
            }
          } catch (e) {
          }
  ...
  const cleanupLocalStorage = () => {
    ...
    localStorage.removeItem('proposalIdForSaglik');
    localStorage.removeItem('selectedQuoteInfo');
    localStorage.removeItem('SaglikQuoteFlow');
        ...
        <Box>
          ...
          <Box sx={{ display: 'grid', gap: 3 }}>
            ...
            <TextField
                ...
                value={cardNumber}
                onChange={(e) => {
                  const digitsOnly = e.target.value.replace(/\D/g, ''); // Sadece rakamları al
                  const truncatedDigits = digitsOnly.slice(0, 16); // 16 ile sınırla
                  const formattedValue = truncatedDigits.replace(/(.{4})/g, '$1 ').trim(); // 4'lü grupla
            ...
            <Box sx={{ display: 'flex', gap: 2 }}>
              <TextField
                  ...
                  onChange={(e) => {
                    ...
                    // Ayın ilk hanesi sadece 0 veya 1 olabilir
                    if (value.length > 0) {
                      if (!['0', '1'].includes(value[0])) {
                        value = ''; // Geçersizse temizle
                      }
                    }
                    
                    // Ay 12'den büyük olamaz (örn: 13, 14...)
                    if (value.length > 1) {
                      const month = parseInt(value.substring(0, 2), 10);
                      if (month > 12) {
                        value = value[0]; // Sadece ilk haneyi tut
                      }
                    }
                    ...
                    setExpiryDate(value.slice(0, 5)); // En fazla 5 karakter (AA/YY)
  ...
  const handlePayment = async () => {
    // Son bir kez tüm formun validasyonunu kontrol et
    const cardNumberValidation = validateCardNumber(cardNumber.replace(/\s/g, ''));
    ...
    if (!cardNumberValidation.isValid || !cardHolderValidation.isValid || !expiryDateValidation.isValid || !cvvValidation.isValid) {
      // Formda hala hata varsa, hata mesajlarını güncelle ve işlemi durdur
      setFormErrors({
        cardNumber: cardNumberValidation.isValid ? '' : cardNumberValidation.message || '',
        cardHolder: cardHolderValidation.isValid ? '' : cardHolderValidation.message || '',
        expiryDate: expiryDateValidation.isValid ? '' : expiryDateValidation.message || '',
        cvv: cvvValidation.isValid ? '' : cvvValidation.message || '',
        cardHolderTckn: '',
      });
      setErrorMessage("Lütfen ödeme formundaki hataları düzeltin.");
      return;
    }
    ...
    let proposalIdFromStorage = localStorage.getItem('proposalIdForSaglik');
    ...
    try {
      ...
      const merchantPaymentId = `Saglik-${proposalIdFromStorage}-${Date.now()}`;
  ...
  const proceedWithPurchase = async (threeDResult: any) => {
    try {
      ...
      const proposalId = localStorage.getItem('proposalIdForSaglik') || selectedQuoteData.proposalId;
      ...
      router.push(`/odeme-sonuc?type=Saglik&success=true&policyId=${result.policyId || ''}`);
      ...
      router.push(`/odeme-sonuc?type=Saglik&success=false`);
    }
  };
  ...
  const handleViewPreInfoForm = async () => {
    ...
    const proposalId = selectedQuoteData.proposalId;
