...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
  ...
  const [tssCaseCreationInProgress, setTssCaseCreationInProgress] = useState(false);
  ...
  // GraphQL Case checking function (TSS)
  ...
  cases(
    ...
    items {
      ...
      if (response.ok) {
        ...
        // Check if there's already a TSS SALE_OPPORTUNITY with OPEN status
        const existingTssCase = cases.find((caseItem: any) => 
          caseItem.productBranch === 'TSS' && 
          caseItem.type === 'SALE_OPPORTUNITY' && 
          caseItem.status === 'OPEN'
        );
        
        return !existingTssCase; // Return true if no existing case found
      }
      return true; // If API fails, allow case creation
    } catch (error) {
      console.warn('GraphQL case kontrol hatası (TSS):', error);
      return true; // If error occurs, allow case creation
    }
  };

  // Case creation function (TSS)
  const createSaleOpportunityCase = async (customerId: string) => {
    ...
    try {
      ...
      if (!canCreateCase) {
        console.log('TSS case zaten mevcut, yeni case oluşturulmadı');
        return;
      }
      ...
      if (response.ok) {
        console.log('TSS case başarıyla oluşturuldu');
      } else {
        console.warn('TSS case oluşturma başarısız:', response.statusText);
      }
    } catch (error) {
      console.warn('TSS case oluşturma hatası:', error);
    }
  };
  ...
  const buildUpdatePayloadForAdditionalInfo = (values: any, customerType: CustomerType, userData: any) => {
    ...
    // Get stored initial data for TSS
    const storedEmail = localStorage.getItem('tssInitialEmail');
    const storedJob = localStorage.getItem('tssInitialJob');
    const storedBirthDate = localStorage.getItem('tssInitialBirthDate');
    
    console.log('=== Building update payload (TSS) ===');
  ...
  useEffect(() => {
    ...
    // Clear case creation flag only if TSS form is starting fresh (no existing completion flag)
    const tssCompleted = localStorage.getItem('tssPersonalInfoCompleted');
    if (!tssCompleted) {
      console.log('=== TSS form starting fresh, clearing case flag ===');
      localStorage.removeItem('tssCaseCreated');
      setTssCaseCreationInProgress(false);
    }
    ...
    const fetchUserDataAndProceed = async () => {
      ...
      try {
        ...
        if (CustomerDataManager.isDataComplete(detectedCustomerType, responseData)) {
          ...
          if (currentCustId) {
            localStorage.setItem('proposalIdForTss', currentCustId);
            
            // Create case for complete customer data (TSS) - only if not already created
            const tssCase = localStorage.getItem('tssCaseCreated');
            console.log('=== handleUpdateCustomerClick case check ===', { tssCase, currentCustId, tssCaseCreationInProgress });
            if (!tssCase && !tssCaseCreationInProgress) {
              setTssCaseCreationInProgress(true);
              localStorage.setItem('tssCaseCreated', 'true'); // Set flag BEFORE attempting creation
              console.log('handleUpdateCustomerClick: Creating TSS case');
              try {
                await createSaleOpportunityCase(currentCustId);
              } catch (error) {
                console.warn('Case oluşturma hatası (TSS):', error);
                localStorage.removeItem('tssCaseCreated'); // Remove flag on error
                setTssCaseCreationInProgress(false);
              }
            }
          }
          // Set flag to prevent AssetInfoStep from going back
          localStorage.setItem('tssPersonalInfoCompleted', 'true');
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo) {
        console.log('=== Storing initial values (TSS) ===');
        ...
        if (customerType === CustomerType.Individual) {
          if (values.email && values.email.trim()) {
            localStorage.setItem('tssInitialEmail', values.email.trim());
            console.log('✅ Saved tssInitialEmail:', values.email.trim());
          }
          if (values.job !== undefined && values.job !== null) {
            localStorage.setItem('tssInitialJob', values.job.toString());
            console.log('✅ Saved tssInitialJob:', values.job.toString(), '(including Unknown/Bilinmiyor)');
          }
          if (values.birthDate && values.birthDate.trim()) {
            localStorage.setItem('tssInitialBirthDate', values.birthDate.trim());
            console.log('✅ Saved tssInitialBirthDate:', values.birthDate.trim());
          }
        } else {
          if (values.email && values.email.trim()) {
            localStorage.setItem('tssInitialEmail', values.email.trim());
            console.log('✅ Saved tssInitialEmail (company):', values.email.trim());
          }
          if (values.birthDate && values.birthDate.trim()) {
            localStorage.setItem('tssInitialBirthDate', values.birthDate.trim());
            console.log('✅ Saved tssInitialBirthDate (company):', values.birthDate.trim());
          }
        }
      }
      ...
      try {
        ...
        if (currentCustomerId) {
          localStorage.setItem('proposalIdForTss', currentCustomerId);
        }
  ...
  const buildUpdatePayload = (values: FormikValues): Record<string, any> => {
    ...
    const emailFromLocalStorage = localStorage.getItem('tssInitialEmail');
    if (emailFromLocalStorage) {
      updatePayload.primaryEmail = emailFromLocalStorage;
    }
    
    const jobFromLocalStorage = localStorage.getItem('tssInitialJob');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const userEnteredEmail = localStorage.getItem('tssInitialEmail');
      const userEnteredJobStr = localStorage.getItem('tssInitialJob');
      ...
      if (isDataComplete && meData) {
        ...
        localStorage.setItem('proposalIdForTss', customerIdToUse);
        
        await updateUserProfileWithCurrentData(meData, userEnteredEmail, userEnteredJob, customerIdToUse);
        
        // Create case for complete customer data (TSS) - only if not already created
        const tssCase = localStorage.getItem('tssCaseCreated');
        console.log('=== handleVerifyOTP case check ===', { tssCase, customerIdToUse, isDataComplete, tssCaseCreationInProgress });
        if (!tssCase && !tssCaseCreationInProgress) {
          setTssCaseCreationInProgress(true);
          localStorage.setItem('tssCaseCreated', 'true'); // Set flag BEFORE attempting creation
          console.log('handleVerifyOTP: Creating TSS case');
          try {
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('proposalIdForTss', customerIdForProposal);
      }
      ...
      // Create case after additional info completion (TSS) - only if not already created
      const tssCase = localStorage.getItem('tssCaseCreated');
      console.log('=== handleSubmitAdditionalInfo case check ===', { tssCase, customerIdForProposal, tssCaseCreationInProgress });
      if (!tssCase && !tssCaseCreationInProgress) {
        setTssCaseCreationInProgress(true);
        localStorage.setItem('tssCaseCreated', 'true'); // Set flag BEFORE attempting creation
        console.log('handleSubmitAdditionalInfo: Creating TSS case');
        try {
      ...
      >
        ...
        >
          ...
          <Typography sx={{ mb: 3, fontSize: '1rem', color: 'text.secondary' }}>
            Kurumsal müşterilerimize Tamamlayıcı Sağlık Sigortası hizmeti verememekteyiz.<br />
    ...
    <Box>
      ...
      {customerType === CustomerType.Company && (
        <Alert 
          ...
          sx={{ mb: 3 }}
        >
          Kurumsal müşterilerimize Tamamlayıcı Sağlık Sigortası hizmeti verememekteyiz. Diğer sigorta ürünlerimizden teklif alabilirsiniz.
        </Alert>
      )}
