...
export default function PurchaseStep({ onNext }: PurchaseStepProps) {
  ...
  const cleanupLocalStorage = () => {
    ...
    localStorage.removeItem('TssQuoteFlow');
  ...
  const handlePayment = async () => {
    ...
    let proposalIdFromStorage = localStorage.getItem('proposalIdForTss');
  ...
  const proceedWithPurchase = async (threeDResult: any) => {
    try {
      ...
      router.push(`/odeme-sonuc?type=Tss&success=true&policyId=${result.policyId || ''}`);
      ...
      router.push(`/odeme-sonuc?type=Tss&success=false`);
    }
  };
