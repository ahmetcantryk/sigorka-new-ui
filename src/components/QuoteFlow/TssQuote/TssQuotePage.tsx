...
// Adımlar
const steps = ['Kişisel Bilgiler', 'Sağlık Bilgileri','Teklif Karşılaştırma','Poliçeleştir'];

// Adım bileşenleri
const stepComponents = [PersonalInfoStep, HealthInfoStep];
...
export default function TssQuotePage() {
        ...
        try {
          ...
          if (response.ok) {
            ...
            setFormData((prev) => ({
              ...
              healthInfo: {
                ...prev.healthInfo,
                gender: profileData.gender || Gender.Unknown,
                maritalStatus: profileData.maritalStatus || MaritalStatus.Unknown,
                nationality: profileData.nationality || Nationality.Unknown,
                educationStatus: profileData.educationStatus || EducationStatus.Unknown,
              },
            }));
  ...
  const handleGetQuotesAndNavigate = async (healthData: Record<string, unknown>): Promise<void> => {
    ...
    try {
      ...
      // Kullanıcının istediği format: coverage null, coverageGroupIds ile default 3 teklif
      const requestData = {
        $type: 'tss',
        insurerCustomerId: customerId,
        insuredCustomerId: customerId,
        coverageGroupIds: getCoverageGroupIds('tss'),
        coverage: null,
        channel: 'WEBSITE',
      };
  ...
  const handleNext = async (stepData?: Record<string, unknown>) => {
    if (activeStep === 0) {
      ...
      if (stepData) {
        updateFormData('healthInfo', stepData);
        await handleGetQuotesAndNavigate(stepData);
      }
    }
  };
  ...
  const getStepProps = (index: number) => {
    ...
    if (index === 0) {
      ...
      commonProps.formData = formData.healthInfo;
    }
