...
const RequestStep = ({ onNext, onBack, isFirstStep, isLastStep }: RequestStepProps) => {
  ...
  useEffect(() => {
    ...
    const errorFromStorage = localStorage.getItem('ozelSaglikRequestError');
    const successFromStorage = localStorage.getItem('ozelSaglikRequestSuccess');

    if (successFromStorage) {
      setRequestResult('success');
      setRequestCreated(true);
      localStorage.removeItem('ozelSaglikRequestSuccess');
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Özel Sağlık Sigortası teklif talebiniz sisteme kaydedildi.<br />
  ...
  if (requestResult === 'existing') {
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Özel Sağlık Sigortası için mevcut bir talebiniz bulunuyor.<br />
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 4 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 3, fontWeight: 'bold' }}>
        Özel Sağlık Sigortası Teklif Talebi
      </Typography>
