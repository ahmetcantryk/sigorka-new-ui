...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
  ...
  const [konutCaseCreationInProgress, setKonutCaseCreationInProgress] = useState(false);
  ...
  // GraphQL Case checking function (KONUT)
...
`
      ...
      if (response.ok) {
        ...
        // Check if there's already a KONUT SALE_OPPORTUNITY with OPEN status
        const existingKonutCase = cases.find((caseItem: any) =>
          caseItem.productBranch === 'KONUT' &&
          caseItem.type === 'SALE_OPPORTUNITY' &&
          caseItem.status === 'OPEN'
        );

        return !existingKonutCase; // Return true if no existing case found
      }
      return true; // If API fails, allow case creation
    } catch (error) {
      console.warn('GraphQL case kontrol hatası (KONUT):', error);
      return true; // If error occurs, allow case creation
    }
  };

  // Case creation function (KONUT)
  const createSaleOpportunityCase = async (customerId: string) => {
    try {
      ...
      if (!canCreateCase) {
        console.log('KONUT case zaten mevcut, yeni case oluşturulmadı');
        return;
      }
      ...
      if (response.ok) {
        console.log('KONUT case başarıyla oluşturuldu');
      } else {
        console.warn('KONUT case oluşturma başarısız:', response.statusText);
      }
    } catch (error) {
      console.warn('KONUT case oluşturma hatası:', error);
    }
  };
  ...
  const buildUpdatePayloadForAdditionalInfo = (values: any, customerType: CustomerType, userData: any) => {
    ...
    // Get stored initial email and job for KONUT
    const storedEmail = localStorage.getItem('konutInitialEmail');
    const storedJob = localStorage.getItem('konutInitialJob');

    console.log('=== Building update payload (KONUT) ===');
  ...
  useEffect(() => {
    ...
    // Clear case creation flag only if KONUT form is starting fresh (no existing completion flag)
    const konutCompleted = localStorage.getItem('konutPersonalInfoCompleted');
    if (!konutCompleted) {
      console.log('=== KONUT form starting fresh, clearing case flag ===');
      localStorage.removeItem('konutCaseCreated');
      setKonutCaseCreationInProgress(false);
    }
    ...
    const fetchUserDataAndProceed = async () => {
      ...
      try {
        ...
        if (CustomerDataManager.isDataComplete(detectedCustomerType, responseData)) {
          ...
          if (currentCustId) {
            localStorage.setItem('proposalIdForKonut', currentCustId);

            // Create case for complete customer data (KONUT) - only if not already created
            const konutCase = localStorage.getItem('konutCaseCreated');
            console.log('=== handleUpdateCustomerClick case check ===', { konutCase, currentCustId, konutCaseCreationInProgress });
            if (!konutCase && !konutCaseCreationInProgress) {
              setKonutCaseCreationInProgress(true);
              localStorage.setItem('konutCaseCreated', 'true'); // Set flag BEFORE attempting creation
              console.log('handleUpdateCustomerClick: Creating KONUT case');
              try {
                await createSaleOpportunityCase(currentCustId);
              } catch (error) {
                console.warn('Case oluşturma hatası (KONUT):', error);
                localStorage.removeItem('konutCaseCreated'); // Remove flag on error
                setKonutCaseCreationInProgress(false);
              }
            }
          }
          // Set flag to prevent PropertyInfoStep from going back
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo) {
        console.log('=== Storing initial values (KONUT) ===');
        ...
        if (customerType === CustomerType.Individual) {
          if (values.email && values.email.trim()) {
            localStorage.setItem('konutInitialEmail', values.email.trim());
            console.log('✅ Saved konutInitialEmail:', values.email.trim());
          }
          if (values.job !== undefined && values.job !== null) {
            localStorage.setItem('konutInitialJob', values.job.toString());
            console.log('✅ Saved konutInitialJob:', values.job.toString(), '(including Unknown/Bilinmiyor)');
          }
        } else {
          if (values.email && values.email.trim()) {
            localStorage.setItem('konutInitialEmail', values.email.trim());
            console.log('✅ Saved konutInitialEmail (company):', values.email.trim());
          }
        }
      }
      ...
      try {
        ...
        if (currentCustomerId) {
          localStorage.setItem('proposalIdForKonut', currentCustomerId);
        }
  ...
  const buildUpdatePayload = (values: FormikValues, customerType: CustomerType): Record<string, any> => {
    ...
    if (customerType === CustomerType.Company) {
      ...
      const emailFromLocalStorage = localStorage.getItem('konutInitialEmail');
      if (emailFromLocalStorage) {
        updatePayload.primaryEmail = emailFromLocalStorage;
      }

      const jobFromLocalStorage = localStorage.getItem('konutInitialJob');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const userEnteredEmail = localStorage.getItem('konutInitialEmail');
      let userEnteredJob = null;

      if (customerType === CustomerType.Individual) {
        const userEnteredJobStr = localStorage.getItem('konutInitialJob');
      ...
      if (isDataComplete && meData) {
        ...
        // Create case for complete customer data (KONUT) - only if not already created
        const konutCase = localStorage.getItem('konutCaseCreated');
        console.log('=== handleVerifyOTP case check ===', { konutCase, customerIdToUse, isDataComplete, konutCaseCreationInProgress });
        if (!konutCase && !konutCaseCreationInProgress) {
          setKonutCaseCreationInProgress(true);
          localStorage.setItem('konutCaseCreated', 'true'); // Set flag BEFORE attempting creation
          console.log('handleVerifyOTP: Creating KONUT case');
          try {
      ...
      // Create case for incomplete customer data (KONUT) - only if not already created
      if (customerIdToUse) {
        const konutCase = localStorage.getItem('konutCaseCreated');
        console.log('=== handleVerifyOTP incomplete data case check ===', { konutCase, customerIdToUse, konutCaseCreationInProgress });
        if (!konutCase && !konutCaseCreationInProgress) {
          setKonutCaseCreationInProgress(true);
          localStorage.setItem('konutCaseCreated', 'true'); // Set flag BEFORE attempting creation
          console.log('handleVerifyOTP incomplete data: Creating KONUT case');
          try {
            await createSaleOpportunityCase(customerIdToUse);
          } catch (error) {
            console.warn('Case oluşturma hatası (KONUT):', error);
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('proposalIdForKonut', customerIdForProposal);
      }
      ...
      // Create case after additional info completion (KONUT) - only if not already created
      const konutCase = localStorage.getItem('konutCaseCreated');
      console.log('=== handleSubmitAdditionalInfo case check ===', { konutCase, customerIdForProposal, konutCaseCreationInProgress });
      if (!konutCase && !konutCaseCreationInProgress) {
        setKonutCaseCreationInProgress(true);
        localStorage.setItem('konutCaseCreated', 'true'); // Set flag BEFORE attempting creation
        console.log('handleSubmitAdditionalInfo: Creating KONUT case');
        try {
