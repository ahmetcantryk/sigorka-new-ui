...
export default function PurchaseStep({ onNext }: PurchaseStepProps) {
    ...
    const check3DResult = () => {
      ...
      if (result2 || status === 'error' || error) {
        ...
        if (status === 'error' || error) {
          ...
          setShowCanvas(false); // Modal'ı kapat
          return;
        }
  ...
  const proceedWithPurchase = async (threeDResult: any) => {
    try {
      ...
      router.push(`/odeme-sonuc?type=Konut&success=true&policyId=${result.policyId || ''}`);
      ...
      router.push(`/odeme-sonuc?type=Konut&success=false`);
    }
  };
