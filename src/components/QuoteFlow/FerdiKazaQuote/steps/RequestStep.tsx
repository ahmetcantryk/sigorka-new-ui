...
const RequestStep = ({ onNext, onBack, isFirstStep, isLastStep }: RequestStepProps) => {
  ...
  useEffect(() => {
    ...
    const errorFromStorage = localStorage.getItem('ferdiKazaRequestError');
    const successFromStorage = localStorage.getItem('ferdiKazaRequestSuccess');

    if (successFromStorage) {
      setRequestResult('success');
      setRequestCreated(true);
      localStorage.removeItem('ferdiKazaRequestSuccess');
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Ferdi Kaza Sigortası teklif talebiniz sisteme kaydedildi.<br />
  ...
  if (requestResult === 'existing') {
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Ferdi Kaza Sigortası için mevcut bir talebiniz bulunuyor.<br />
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 4 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 3, fontWeight: 'bold' }}>
        Ferdi Kaza Sigortası Teklif Talebi
      </Typography>
