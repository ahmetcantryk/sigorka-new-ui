...
export default function PurchaseStep({ onNext, proposalId: propProposalId }: PurchaseStepProps) {
      ...
      if (result2 || status === 'error' || error) {
        ...
        if (result2) {
          try {
            ...
            if (parsed.success) {
              localStorage.removeItem('paratika_3d_result');
              router.push('/odeme/paratika-callback?type=seyahat-saglik&action=validate');
              return;
            }
          } catch (e) {
          }
  ...
  const cleanupLocalStorage = () => {
    ...
    localStorage.removeItem('SeyahatSaglikProposalId');
    localStorage.removeItem('currentProposalId');
    localStorage.removeItem('selectedQuoteInfo');
    localStorage.removeItem('seyahatSaglikQuoteFlow');
          ...
          <>
            ...
            <Box sx={{ mb: 2 }}>
              <Typography variant="h6" color="primary">
                ...
                {getCurrencySymbol(currentPremium.currency)}
              </Typography>
  ...
  const handlePayment = async () => {
    ...
    let proposalIdFromStorage = localStorage.getItem('SeyahatSaglikProposalId');
    if (!proposalIdFromStorage) {
        proposalIdFromStorage = propProposalId || selectedQuoteData.proposalId;
    }
    ...
    try {
      ...
      const merchantPaymentId = `SEYAHAT-SAGLIK-${proposalIdFromStorage}-${Date.now()}`;
        ...
        orderItems: [
          {
            ...
            description: `${proposalIdFromStorage} numaralı seyahat sağlık sigorta teklifi`,
            quantity: 1,
            amount: currentPremium.grossPremium,
          },
        ],
  ...
  const proceedWithPurchase = async (threeDResult: any) => {
    try {
      ...
      let proposalId = localStorage.getItem('SeyahatSaglikProposalId') || propProposalId;
      if (!proposalId) {
        proposalId = selectedQuoteData.proposalId;
      }
      ...
      router.push(`/odeme-sonuc?type=SeyahatSaglik&success=true&policyId=${result.policyId || ''}`);
      ...
      router.push(`/odeme-sonuc?type=SeyahatSaglik&success=false`);
    }
  };
  ...
  const handleViewPreInfoForm = async () => {
    ...
    let proposalId = localStorage.getItem('SeyahatSaglikProposalId') || propProposalId;
    if (!proposalId && selectedQuoteData) {
      proposalId = selectedQuoteData.proposalId;
    }
