...
export default function QuoteComparisonStep({
  ...
  const proposalIdToUse = initialProposalId || params?.proposalId as string | undefined || localStorage.getItem('SeyahatSaglikProposalId');

  useEffect(() => {
    const storedProposalId = localStorage.getItem('SeyahatSaglikProposalId');
    if (storedProposalId) {
      ...
      setError('Seyahat sağlık teklif ID bilgisi bulunamadı. Lütfen önceki adıma dönüp tekrar deneyin.');
      setIsLoading(false);
    }
  }, [params.proposalId]);
  ...
  useEffect(() => {
    ...
    const fetchQuotes = async (currentCompanies: InsuranceCompany[]) => {
      ...
      try {
        ...
        const allowedProductIds = agencyConfig.homepage.partners.companies.flatMap((c: any) => c.products['seyahat-saglik'] || []);
        ...
        if (allRelevantQuotesFinalized || timeoutReached) {
          ...
          if (hasSuccessfulQuotes) {
            pushToDataLayer({
              ...
              form_name: "seyahat_saglik_teklif_basarili"
            });
          } else {
            pushToDataLayer({
              event: "seyahat_saglik_formsubmit",
              form_name: "seyahat_saglik_teklif_basarisiz"
            });
          }
    ...
    const startPolling = async () => {
      ...
      try {
        ...
        // Polling başlat - 5 saniye aralıklarla (Kasko gibi)
  ...
  const filterQuotesByProductIds = (quotes: ProcessedQuote[]) => {
    const allowedProductIds = agencyConfig.homepage.partners.companies.flatMap((company: any) => 
      company.products['seyahat-saglik'] || []
    );
  ...
  const handlePurchase = (quoteId: string) => {
    ...
    if (selectedFullQuote && selectedFullQuote.state === 'ACTIVE') {
      ...
      // onPurchaseClick prop'u varsa onu kullan, yoksa router.push kullan
      if (onPurchaseClick) {
        onPurchaseClick(quoteId);
      } else {
        router.push(`/seyahat-saglik/purchase/${proposalId}/${quoteId}`);
      }
    } else {
      ...
      <>
        <Box sx={{ mb: 4 }}>
          <Typography variant="h5" component="h1" fontWeight="600" gutterBottom>
            Seyahat Sağlık Sigortası Teklifleri
          </Typography>
          <Typography variant="body1" color="text.secondary" sx={{ mb: 1 }}>
            Size en uygun Seyahat Sağlık Sigortası teklifini seçip hemen satın alabilirsiniz
          </Typography>
                                    ...
                                    <>
                                      <Box sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
                                        <Typography
                                            ...
                                            sx={{ mr: 1 }}
                                        >
                                          {currentPremium?.formattedGrossPremium && currentPremium?.currency
                                              ? `${currentPremium.formattedGrossPremium} ${getCurrencySymbol(currentPremium.currency)}`
                                              : 'Fiyat Yok'}
                                        </Typography>
                                                ...
                                                >
                                                  <Typography variant="body2">
                                                    {premium.installmentNumber === 1
                                                        ? `Peşin: ${premium.formattedGrossPremium} ${getCurrencySymbol(premium.currency)}`
                                                        : `${premium.installmentNumber} Taksit: ${premium.formattedGrossPremium} ${getCurrencySymbol(premium.currency)}`}
                                                  </Typography>
                                                </MenuItem>
                                            ))}
                                          </Select>
        ...
        <Box sx={{ mt: 4, p: 2, bgcolor: 'background.paper', borderRadius: 2, border: '1px solid', borderColor: 'divider' }}>
          <Typography variant="body2" color="text.secondary">
            <Box component="span" sx={{ display: 'flex', alignItems: 'center', mb: 1 }}>
              ...
              <Typography variant="subtitle2" component="span">Seyahat Sağlık Sigortası Hakkında</Typography>
            </Box>
            Seyahat Sağlık Sigortası, yurtdışı seyahatlerinizde karşılaşabileceğiniz sağlık sorunlarınızı karşılar. Acil tıbbi müdahale, hastane yatışı, ilaç ve tıbbi nakil gibi hizmetleri güvence altına alır. Size en uygun Seyahat Sağlık Sigortası teklifini seçerek hemen satın alabilirsiniz.
          </Typography>
        </Box>
                  ...
                  <Box sx={{ 
                    ...
                    <Box sx={{ textAlign: { xs: 'left', sm: 'right' }, mt: { xs: 1, sm: 0 } }}>
                      <Typography variant="h6" color="primary.main" fontWeight="bold" sx={{ fontSize: { xs: '1.125rem', sm: '1.25rem' } }}>
                        {getSelectedPremium(selectedQuoteForModal)?.formattedGrossPremium} {getSelectedPremium(selectedQuoteForModal)?.currency ? getCurrencySymbol(getSelectedPremium(selectedQuoteForModal)!.currency) : '₺'}
                      </Typography>
