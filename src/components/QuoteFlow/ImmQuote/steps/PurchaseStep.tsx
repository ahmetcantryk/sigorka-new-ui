...
export default function PurchaseStep({ onNext }: PurchaseStepProps) {
  ...
  const cleanupLocalStorage = () => {
    ...
    localStorage.removeItem('proposalIdForImm');
    localStorage.removeItem('selectedQuoteInfo');
    localStorage.removeItem('ImmQuoteFlow');
  ...
  const handlePayment = async () => {
    ...
    let proposalIdFromStorage = localStorage.getItem('proposalIdForImm');
        ...
        orderItems: [
          {
            ...
            name: 'Imm Sigortası',
            description: `${proposalIdFromStorage} numaralı Imm sigorta teklifi`,
            quantity: 1,
            amount: currentPremium.grossPremium,
          },
        ],
  ...
  const proceedWithPurchase = async (threeDResult: any) => {
    try {
      ...
      router.push(`/odeme-sonuc?type=Imm&success=true&policyId=${result.policyId || ''}`);
      ...
      router.push(`/odeme-sonuc?type=Imm&success=false`);
    }
  };
