...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
  ...
  const [immCaseCreationInProgress, setImmCaseCreationInProgress] = useState(false);
  ...
  useEffect(() => {
    ...
    // Clear case creation flag only if IMM form is starting fresh (no existing completion flag)
    const immCompleted = localStorage.getItem('immPersonalInfoCompleted');
    if (!immCompleted) {
      console.log('=== IMM form starting fresh, clearing case flag ===');
      localStorage.removeItem('immCaseCreated');
      setImmCaseCreationInProgress(false);
    }
    ...
    const fetchUserDataAndProceed = async () => {
      ...
      try {
        ...
        if (CustomerDataManager.isDataComplete(detectedCustomerType, responseData)) {
            ...
            if (currentCustId) {
              localStorage.setItem('proposalIdForImm', currentCustId);
              
              // Create case for complete customer data (IMM) - only if not already created
              const immCase = localStorage.getItem('immCaseCreated');
              console.log('=== handleUpdateCustomerClick case check ===', { immCase, currentCustId, immCaseCreationInProgress });
              if (!immCase && !immCaseCreationInProgress) {
                setImmCaseCreationInProgress(true);
                localStorage.setItem('immCaseCreated', 'true'); // Set flag BEFORE attempting creation
                console.log('handleUpdateCustomerClick: Creating IMM case');
                try {
                  await createSaleOpportunityCase(currentCustId);
                } catch (error) {
                  console.warn('Case oluşturma hatası (IMM):', error);
                  localStorage.removeItem('immCaseCreated'); // Remove flag on error
                  setImmCaseCreationInProgress(false);
                }
              }
            }
          // Set flag to prevent AssetInfoStep from going back
          localStorage.setItem('immPersonalInfoCompleted', 'true');
  ...
  const updateUserProfileWithCurrentData = async (
    ...
    try {
      ...
      if (userJob !== undefined && userJob !== null && !isNaN(userJob)) {
        updatePayload.job = userJob;
        console.log('✅ Added job to payload (IMM):', userJob, '(including Unknown/Bilinmiyor if 0)');
      } else if (currentMeData.job !== undefined && currentMeData.job !== null) {
        updatePayload.job = currentMeData.job;
        console.log('✅ Added job from currentMeData (IMM):', currentMeData.job);
      }
      ...
      console.warn('Profile update hatası (IMM):', updateError);
    }
  };
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo) {
        console.log('=== Storing initial values (IMM) ===');
        ...
        if (customerType === CustomerType.Individual) {
          if (values.email && values.email.trim()) {
            localStorage.setItem('immInitialEmail', values.email.trim());
            console.log('✅ Saved immInitialEmail:', values.email.trim());
          }
          if (values.job !== undefined && values.job !== null) {
            localStorage.setItem('immInitialJob', values.job.toString());
            console.log('✅ Saved immInitialJob:', values.job.toString(), '(including Unknown/Bilinmiyor)');
          }
        } else {
          if (values.email && values.email.trim()) {
            localStorage.setItem('immInitialEmail', values.email.trim());
            console.log('✅ Saved immInitialEmail (company):', values.email.trim());
          }
        }
      }
      ...
      try {
        ...
        if (currentCustomerId) {
          localStorage.setItem('proposalIdForImm', currentCustomerId);
          
          // Create case for complete customer data (IMM) - only if not already created
          const immCase = localStorage.getItem('immCaseCreated');
          console.log('=== formik.onSubmit case check ===', { immCase, currentCustomerId, immCaseCreationInProgress });
          if (!immCase && !immCaseCreationInProgress) {
            setImmCaseCreationInProgress(true);
            localStorage.setItem('immCaseCreated', 'true'); // Set flag BEFORE attempting creation
            console.log('formik.onSubmit: Creating IMM case');
            try {
  ...
  // Fetch customer cases via GraphQL (IMM)
  ...
  cases(
    ...
    items {
      ...
      console.log('=== Customer Cases GraphQL Response (IMM) ===');
      console.log('customerId:', customerId);
      console.log('totalCount:', data.data?.cases?.totalCount);
      console.log('cases:', data.data?.cases?.items);
      
      // Check for existing IMM SALE_OPPORTUNITY with OPEN status
      const immOpenSaleOpportunities = data.data?.cases?.items?.filter((caseItem: any) => 
        caseItem.productBranch === 'IMM' && 
        caseItem.type === 'SALE_OPPORTUNITY' &&
        caseItem.status === 'OPEN'
      );
      
      console.log('IMM SALE_OPPORTUNITY + OPEN cases:', immOpenSaleOpportunities);
      console.log('Found', immOpenSaleOpportunities?.length || 0, 'existing IMM SALE_OPPORTUNITY + OPEN cases');
      
      return {
        cases: data.data?.cases,
        hasOpenImmSaleOpportunity: immOpenSaleOpportunities && immOpenSaleOpportunities.length > 0
      };
    } catch (error) {
      console.error('GraphQL cases query hatası (IMM):', error);
      throw error;
    }
  };

  // Create sale opportunity case with duplicate check (IMM)
  const createSaleOpportunityCase = async (customerId: string) => {
    try {
      // First check if there's already an open IMM sale opportunity
      console.log('🔍 Checking for existing IMM SALE_OPPORTUNITY + OPEN cases...');
      const casesResult = await fetchCases(customerId);
      
      if (casesResult.hasOpenImmSaleOpportunity) {
        console.log('❌ IMM SALE_OPPORTUNITY + OPEN case already exists. Skipping case creation.');
        return { skipped: true, reason: 'Open IMM sale opportunity already exists' };
      }
      
      console.log('✅ No existing IMM SALE_OPPORTUNITY + OPEN cases found. Creating new case...');
      ...
      console.log('✅ IMM Case oluşturuldu:', caseData);
      return caseData;
    } catch (error) {
      console.error('IMM Case oluşturma hatası:', error);
      throw error;
    }
  };
  ...
  const buildUpdatePayload = (values: FormikValues, customerType: CustomerType): Record<string, any> => {
    ...
    if (customerType === CustomerType.Company) {
      ...
      const emailFromLocalStorage = localStorage.getItem('immInitialEmail');
      if (emailFromLocalStorage) {
        updatePayload.primaryEmail = emailFromLocalStorage;
      }
      
      const jobFromLocalStorage = localStorage.getItem('immInitialJob');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const userEnteredEmail = localStorage.getItem('immInitialEmail');
      let userEnteredJob = null;
      
      if (customerType === CustomerType.Individual) {
      const userEnteredJobStr = localStorage.getItem('immInitialJob');
      ...
      if (isDataComplete && meData) {
        ...
        localStorage.setItem('proposalIdForImm', customerIdToUse);
        ...
        // Create case for complete customer data (IMM) - only if not already created
        const immCase = localStorage.getItem('immCaseCreated');
        console.log('=== handleVerifyOTP case check ===', { immCase, customerIdToUse, isDataComplete, immCaseCreationInProgress });
        if (!immCase && !immCaseCreationInProgress) {
          setImmCaseCreationInProgress(true);
          localStorage.setItem('immCaseCreated', 'true'); // Set flag BEFORE attempting creation
          console.log('handleVerifyOTP: Creating IMM case');
          try {
      ...
      if (customerIdToUse) {
        ...
        // Update email and job even if data is not complete (IMM)
        if (customerType === CustomerType.Individual && (userEnteredEmail || userEnteredJob)) {
          try {
            await updateUserProfileWithCurrentData(meData, userEnteredEmail, userEnteredJob, customerIdToUse);
          } catch (error) {
            console.warn('Email/Job update hatası (IMM):', error);
          }
        }
        }
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('proposalIdForImm', customerIdForProposal);
        
        // Create case after additional info completion (IMM) - only if not already created
        const immCase = localStorage.getItem('immCaseCreated');
        console.log('=== handleSubmitAdditionalInfo case check ===', { immCase, customerIdForProposal, immCaseCreationInProgress });
        if (!immCase && !immCaseCreationInProgress) {
          setImmCaseCreationInProgress(true);
          localStorage.setItem('immCaseCreated', 'true'); // Set flag BEFORE attempting creation
          console.log('handleSubmitAdditionalInfo: Creating IMM case');
          try {
  ...
  const buildUpdatePayloadForAdditionalInfo = (values: FormikValues, currentMeData: CustomerProfile, customerType: CustomerType): Record<string, any> => {
    const userEnteredEmail = values.email || localStorage.getItem('immInitialEmail');
    const userEnteredJobStr = localStorage.getItem('immInitialJob');
    
    console.log('=== buildUpdatePayloadForAdditionalInfo Debug (IMM) ===');
    console.log('values.email:', values.email);
    console.log('localStorage immInitialEmail:', localStorage.getItem('immInitialEmail'));
    console.log('localStorage immInitialJob:', localStorage.getItem('immInitialJob'));
    ...
    if (customerType === CustomerType.Individual) {
      ...
      if (userEnteredJob !== undefined && userEnteredJob !== null && !isNaN(userEnteredJob)) {
        ...
        console.log('✅ Added job to payload (IMM):', userEnteredJob, '(including Unknown/Bilinmiyor if 0)');
      } else if (currentMeData.job !== undefined && currentMeData.job !== null) {
        updatePayload.job = currentMeData.job;
        console.log('✅ Added job from currentMeData (IMM):', currentMeData.job);
      }
    ...
    console.log('Final updatePayload (IMM):', updatePayload);
  ...
  const renderAdditionalInfoForm = () => (
    <Box component="form" onSubmit={formik.handleSubmit} noValidate>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        İMM Sigortası teklifiniz için eksik bilgilerinizi doldurunuz
      </Typography>
