...
export default function AssetInfoStep({
  ...
  useEffect(() => {
    const checkMissingInfo = async () => {
      if (accessToken && customerId) {
        ...
        const personalInfoCompleted = localStorage.getItem('trafikPersonalInfoCompleted');
        if (personalInfoCompleted === 'true') {
          // Clear the flag and skip the check
          localStorage.removeItem('trafikPersonalInfoCompleted');
          return;
        }
        ...
        try {
          ...
          if (response.ok) {
            ...
            const hasMissingInfo = (!data.fullName && !data.title) || !data.city?.value || !data.district?.value;
  ...
  const fetchWithAuthModels = useCallback(async (brandCode: string, modelYear: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setVehicleModels(sortedModels.map(m => ({ ...m, label: m.text })));
      } else {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
        if (selectionType === 'existing') {
            ...
            localStorage.setItem('selectedVehicleIdForTrafik', selectedVehicleId);
  ...
  const handleExistingVehicleProposal = async () => {
    ...
    try {
      ...
      setNotificationMessage(`Trafik sigorta teklifi oluşturulurken bir hata oluştu: ${(error as Error).message}`);
      setNotificationSeverity('error');
      setShowNotification(true);
    } finally {
      setIsLoading(false);
    ...
    try {
      ...
      if (proposalResponse.ok) {
        ...
        if (proposalResult && proposalResult.proposalId) {
          localStorage.setItem('proposalIdForTrafik', proposalResult.proposalId);
        ...
        throw new Error('Trafik sigorta teklifi oluşturulurken bir hata oluştu');
      }
    } catch (error) {
      setNotificationMessage(`Trafik sigorta teklifi oluşturulurken bir hata oluştu: ${(error as Error).message}`);
      setNotificationSeverity('error');
    ...
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        {selectionType === 'existing'
          ...
          : 'Trafik Sigortası teklifiniz için araç bilgilerinizi giriniz'}
      </Typography>
