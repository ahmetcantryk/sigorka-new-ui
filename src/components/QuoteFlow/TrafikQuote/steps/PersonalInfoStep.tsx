...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
  ...
  const [trafikCaseCreationInProgress, setTrafikCaseCreationInProgress] = useState(false);
  ...
  useEffect(() => {
    ...
    // Clear case creation flag when TRAFIK form starts fresh
    const trafikCompleted = localStorage.getItem('trafikPersonalInfoCompleted');
    if (!trafikCompleted) {
      localStorage.removeItem('trafikCaseCreated');
      setTrafikCaseCreationInProgress(false);
    }
    ...
    const fetchUserDataAndProceed = async () => {
      ...
      try {
        ...
        if (CustomerDataManager.isDataComplete(detectedCustomerType, responseData)) {
          ...
          if (currentCustId) {
            localStorage.setItem('proposalIdForTrafik', currentCustId);
            
            // Create case for complete customer data (TRAFIK) - only if not already created
            const trafikCase = localStorage.getItem('trafikCaseCreated');
            if (!trafikCase && !trafikCaseCreationInProgress) {
              setTrafikCaseCreationInProgress(true);
              localStorage.setItem('trafikCaseCreated', 'true');
              try {
                await createSaleOpportunityCase(currentCustId);
              } catch (error) {
                console.warn('Case oluşturma hatası (TRAFIK):', error);
                localStorage.removeItem('trafikCaseCreated');
                setTrafikCaseCreationInProgress(false);
              }
            }
          ...
          localStorage.setItem('trafikPersonalInfoCompleted', 'true');
  ...
  const updateUserProfileWithCurrentData = async (
    ...
    try {
      ...
      if (userJob !== undefined && userJob !== null && !isNaN(userJob)) {
        ...
        console.log('✅ Added job to payload (TRAFIK):', userJob, '(including Unknown/Bilinmiyor if 0)');
      } else if (currentMeData.job !== undefined && currentMeData.job !== null) {
        updatePayload.job = currentMeData.job;
        console.log('✅ Added job from currentMeData (TRAFIK):', currentMeData.job);
      }
      ...
      console.warn('Profile update hatası (TRAFIK):', updateError);
    }
  };
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo) {
        console.log('=== Storing initial values (Trafik) ===');
        ...
        if (customerType === CustomerType.Individual) {
          if (values.email && values.email.trim()) {
            localStorage.setItem('trafikInitialEmail', values.email.trim());
            console.log('✅ Saved trafikInitialEmail:', values.email.trim());
          }
          if (values.job !== undefined && values.job !== null) {
            localStorage.setItem('trafikInitialJob', values.job.toString());
            console.log('✅ Saved trafikInitialJob:', values.job.toString(), '(including Unknown/Bilinmiyor)');
          }
        } else {
          if (values.email && values.email.trim()) {
            localStorage.setItem('trafikInitialEmail', values.email.trim());
            console.log('✅ Saved trafikInitialEmail (company):', values.email.trim());
          }
        }
      }
      ...
      try {
        ...
        if (currentCustomerId) {
          localStorage.setItem('proposalIdForTrafik', currentCustomerId);
  ...
  // Fetch customer cases via GraphQL (TRAFIK)
  ...
  cases(
    ...
    items {
      ...
      console.log('=== Customer Cases GraphQL Response (TRAFIK) ===');
      console.log('customerId:', customerId);
      console.log('totalCount:', data.data?.cases?.totalCount);
      console.log('cases:', data.data?.cases?.items);
      
      // Check for existing TRAFIK SALE_OPPORTUNITY with OPEN status
      const trafikOpenSaleOpportunities = data.data?.cases?.items?.filter((caseItem: any) => 
        caseItem.productBranch === 'TRAFIK' && 
        caseItem.type === 'SALE_OPPORTUNITY' &&
        caseItem.status === 'OPEN'
      );
      
      console.log('TRAFIK SALE_OPPORTUNITY + OPEN cases:', trafikOpenSaleOpportunities);
      console.log('Found', trafikOpenSaleOpportunities?.length || 0, 'existing TRAFIK SALE_OPPORTUNITY + OPEN cases');
      
      return {
        cases: data.data?.cases,
        hasOpenTrafikSaleOpportunity: trafikOpenSaleOpportunities && trafikOpenSaleOpportunities.length > 0
      };
    } catch (error) {
      console.error('GraphQL cases query hatası (TRAFIK):', error);
      throw error;
    }
  };

  // Create sale opportunity case with duplicate check (TRAFIK)
  const createSaleOpportunityCase = async (customerId: string) => {
    try {
      // First check if there's already an open TRAFIK sale opportunity
      console.log('🔍 Checking for existing TRAFIK SALE_OPPORTUNITY + OPEN cases...');
      const casesResult = await fetchCases(customerId);
      
      if (casesResult.hasOpenTrafikSaleOpportunity) {
        console.log('❌ TRAFIK SALE_OPPORTUNITY + OPEN case already exists. Skipping case creation.');
        return { skipped: true, reason: 'Open TRAFIK sale opportunity already exists' };
      }
      
      console.log('✅ No existing TRAFIK SALE_OPPORTUNITY + OPEN cases found. Creating new case...');
      ...
      console.log('✅ TRAFIK Case oluşturuldu:', caseData);
      return caseData;
    } catch (error) {
      console.error('TRAFIK Case oluşturma hatası:', error);
      throw error;
    }
  };
  ...
  const buildUpdatePayload = (values: FormikValues, customerType: CustomerType): Record<string, any> => {
    ...
    if (customerType === CustomerType.Company) {
      ...
      const emailFromLocalStorage = localStorage.getItem('trafikInitialEmail');
      if (emailFromLocalStorage) {
        updatePayload.primaryEmail = emailFromLocalStorage;
      }
      
      const jobFromLocalStorage = localStorage.getItem('trafikInitialJob');
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      const userEnteredEmail = localStorage.getItem('trafikInitialEmail');
      let userEnteredJob = null;
      
      if (customerType === CustomerType.Individual) {
        const userEnteredJobStr = localStorage.getItem('trafikInitialJob');
      ...
      if (isDataComplete && meData) {
        ...
        setCustomerId(customerIdToUse);
            localStorage.setItem('proposalIdForTrafik', customerIdToUse);
      ...
      if (customerIdToUse) {
        ...
        // Update email and job even if data is not complete (TRAFIK)
        if (customerType === CustomerType.Individual && (userEnteredEmail || userEnteredJob)) {
          try {
            await updateUserProfileWithCurrentData(meData, userEnteredEmail, userEnteredJob, customerIdToUse);
          } catch (error) {
            console.warn('Email/Job update hatası (TRAFIK):', error);
          }
        }
        }
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('proposalIdForTrafik', customerIdForProposal);
        
        // Create case for complete customer data after additional info completion (TRAFIK) - only if not already created
        const trafikCase = localStorage.getItem('trafikCaseCreated');
        if (!trafikCase && !trafikCaseCreationInProgress) {
          setTrafikCaseCreationInProgress(true);
          localStorage.setItem('trafikCaseCreated', 'true');
  ...
  const buildUpdatePayloadForAdditionalInfo = (values: FormikValues, currentMeData: CustomerProfile, customerType: CustomerType): Record<string, any> => {
    const userEnteredEmail = values.email || localStorage.getItem('trafikInitialEmail');
    const userEnteredJobStr = localStorage.getItem('trafikInitialJob');
    
    console.log('=== buildUpdatePayloadForAdditionalInfo Debug (TRAFIK) ===');
    console.log('values.email:', values.email);
    console.log('localStorage trafikInitialEmail:', localStorage.getItem('trafikInitialEmail'));
    console.log('localStorage trafikInitialJob:', localStorage.getItem('trafikInitialJob'));
    ...
    if (customerType === CustomerType.Individual) {
      ...
      if (userEnteredJob !== undefined && userEnteredJob !== null && !isNaN(userEnteredJob)) {
        ...
        console.log('✅ Added job to payload (TRAFIK):', userEnteredJob, '(including Unknown/Bilinmiyor if 0)');
      } else if (currentMeData.job !== undefined && currentMeData.job !== null) {
        updatePayload.job = currentMeData.job;
        console.log('✅ Added job from currentMeData (TRAFIK):', currentMeData.job);
      }
    ...
    console.log('Final updatePayload (TRAFIK):', updatePayload);
  ...
  const renderAdditionalInfoForm = () => (
    <Box component="form" onSubmit={formik.handleSubmit} noValidate>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Trafik Sigortası teklifiniz için eksik bilgilerinizi doldurunuz
      </Typography>
