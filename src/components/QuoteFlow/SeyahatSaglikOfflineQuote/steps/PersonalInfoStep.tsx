...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
      ...
      try {
        ...
        if (!fullName || !city || !district) {
          ...
          if (isCoreDataComplete) {
            ...
            if (currentCustId) {
              localStorage.setItem('seyahatSaglikProposalId', currentCustId);
            }
            // Otomatik step geçişi - analytics step event
            pushToDataLayer({
              event: "seyahat_saglik_formsubmit",
              form_name: "seyahat_saglik_step1"
            });
            onNext();
          } else {
            setShowAdditionalInfo(true);
  ...
  const fetchWithAuthCities = async (isMountedParam?: boolean) => {
    ...
    try {
      ...
      if (!rawResponse.ok) {
        ...
        throw new Error(`Şehirler alınamadı: ${rawResponse.status}`);
      }
  ...
  const fetchWithAuthDistricts = async (cityValue: string, isMountedParam?: boolean) => {
    ...
    try {
      ...
      if (!rawResponse.ok) {
        ...
        throw new Error(`İlçeler yüklenemedi: ${rawResponse.status}`);
      }
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo && values.email && values.job !== Job.Unknown) {
        localStorage.setItem('seyahatSaglikInitialEmail', values.email);
        localStorage.setItem('seyahatSaglikInitialJob', values.job.toString());
      }
      ...
      try {
        ...
        const emailFromLocalStorage = localStorage.getItem('seyahatSaglikInitialEmail');
        if (emailFromLocalStorage) {
          updatePayload.primaryEmail = emailFromLocalStorage;
        }
        
        // Job'u localStorage'dan al ve Unknown değilse gönder
        const jobFromLocalStorage = localStorage.getItem('seyahatSaglikInitialJob');
        ...
        if (currentCustomerId) {
            localStorage.setItem('seyahatSaglikProposalId', currentCustomerId);
        } else {
        }
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (meData) {
        ...
        const storedEmail = localStorage.getItem('seyahatSaglikInitialEmail');
        const storedJob = localStorage.getItem('seyahatSaglikInitialJob');
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      const updatedProfile = await updateCustomerProfile(updatePayload, currentProfile.id);
      
             // updateCustomerProfile zaten güncellenmiş profili döndürür
       let finalUpdatedProfile: CustomerProfile = updatedProfile;
      
      const customerIdForProposal = getCustomerIdFromAuthStorage() || finalUpdatedProfile.id; 
      if (customerIdForProposal) {
        localStorage.setItem('seyahatSaglikProposalId', customerIdForProposal);
      } else {
      }
  ...
  const handleCreateRequestInPersonalInfo = async (tokenToUse?: string) => {
    try {
      ...
      if (!currentCustomerId) {
        ...
        localStorage.setItem('seyahatSaglikRequestError', 'Müşteri bilgisi bulunamadı. Lütfen önceki adımları kontrol edin.');
        onNext();
        return;
      }
      ...
      if (!currentToken) {
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('seyahatSaglikRequestError', 'Kimlik doğrulama sorunu. Lütfen sayfayı yenileyin.');
        onNext();
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
            ...
            if (apiError.includes('zaten açık bir yeni satış fırsatı talebi bulunmaktadır')) {
              errorMessage = 'Bu müşteri için zaten açık bir Seyahat Sağlık Sigortası talebi bulunmaktadır. Mevcut talebin sonuçlanmasını bekleyiniz.';
            } else {
        ...
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('seyahatSaglikRequestError', errorMessage);
        onNext();
        return;
      }
      ...
      localStorage.setItem('seyahatSaglikRequestSuccess', 'true');
      ...
      // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
      localStorage.setItem('seyahatSaglikRequestError', error instanceof Error ? error.message : 'Talep oluşturulurken bir hata oluştu');
      onNext();
    } finally {
      setIsLoading(false);
    }
      ...
      >
        ...
        >
          ...
          <Typography sx={{ mb: 3, fontSize: '1rem', color: 'text.secondary' }}>
            Kurumsal müşterilerimize Seyahat Sağlık Sigortası hizmeti verememekteyiz.<br />
    ...
    return (
      <Box component="form" onSubmit={formik.handleSubmit} noValidate>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Seyahat Sağlık Sigortası teklifiniz için eksik bilgilerinizi doldurunuz
        </Typography>
