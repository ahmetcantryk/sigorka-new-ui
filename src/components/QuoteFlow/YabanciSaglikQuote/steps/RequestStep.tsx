...
const RequestStep = ({ onNext, onBack, isFirstStep, isLastStep }: RequestStepProps) => {
  ...
  useEffect(() => {
    ...
    const errorFromStorage = localStorage.getItem('yabanciSaglikRequestError');
    const successFromStorage = localStorage.getItem('yabanciSaglikRequestSuccess');

    if (successFromStorage) {
      setRequestResult('success');
      setRequestCreated(true);
      localStorage.removeItem('yabanciSaglikRequestSuccess');
    ...
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Yabancı Sağlık Sigortası teklif talebiniz sisteme kaydedildi.<br />
  ...
  if (requestResult === 'existing') {
    return (
      <Box sx={{ textAlign: 'center', py: 4, paddingBottom: 10 }}>
        ...
        <Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
          Yabancı Sağlık Sigortası için mevcut bir talebiniz bulunuyor.<br />
  ...
  return (
    <Box sx={{ textAlign: 'center', py: 4 }}>
      <Typography variant="h5" gutterBottom sx={{ mb: 3, fontWeight: 'bold' }}>
        Yabancı Sağlık Sigortası Teklif Talebi
      </Typography>
