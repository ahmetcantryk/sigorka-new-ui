...
const additionalInfoValidationSchema = yup.object({
  ...
  // KVKK onayı ilk stepte alındığı için burada gerekli değil
});
...
const PersonalInfoStep = ({ onNext, onBack, isFirstStep, isLastStep }: PersonalInfoStepProps) => {
  ...
  useEffect(() => {
    ...
    const fetchUserDataAndProceed = async () => {
      if (!accessToken) { 
        // If no access token, user might need to fill form or it's initial load without login
      ...
      try {
        ...
        if (!rawResponse.ok) {
          ...
          // If CUSTOMER_ME fails (e.g. 401 or 404), don't throw error that stops the page,
          // let the user fill the form. setError might be too disruptive here.
          // Instead, console.warn and proceed to let user fill form.
          // Cities will be fetched when showAdditionalInfo becomes true
          setIsLoading(false);
          return; 
        }
        ...
        if (!fullName || !city || !district) {
          ...
          if (isCoreDataComplete) {
            ...
            if (currentCustId) {
              localStorage.setItem('yabanciSaglikProposalId', currentCustId);
            }
            // Otomatik step geçişi - analytics step event
            pushToDataLayer({
              event: "yabancisaglik_formsubmit",
              form_name: "yabancisaglik_step1"
            });
            // Müşteri bilgileri tam, otomatik talep oluştur
  ...
  }, [accessToken]); // showAdditionalInfo çıkarıldı, çünkü bu useEffect sadece token'a bağlı olmalı ve /me çekmeli. Eksik bilgi formu /me yanıtına göre gösterilecek.
  ...
  const formik = useFormik<FormikValues>({
    ...
    onSubmit: async (values) => {
      ...
      if (!showAdditionalInfo && values.email && values.job !== Job.Unknown) {
        localStorage.setItem('yabanciSaglikInitialEmail', values.email);
        localStorage.setItem('yabanciSaglikInitialJob', values.job.toString());
      }
      ...
      try {
        ...
        const emailFromLocalStorage = localStorage.getItem('yabanciSaglikInitialEmail');
        if (emailFromLocalStorage) {
          updatePayload.primaryEmail = emailFromLocalStorage;
        }
        
        // Job'u localStorage'dan al ve Unknown değilse gönder
        const jobFromLocalStorage = localStorage.getItem('yabanciSaglikInitialJob');
        ...
        // Set proposalId using the customerId from auth-storage (which is currentCustomerId)
        if (currentCustomerId) {
            localStorage.setItem('yabanciSaglikProposalId', currentCustomerId);
        } else {
        }
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // The authStore should be responsible for persisting to 'auth-storage'.
      ...
      // Kullanıcı detaylarını çek ve forma doldur/sonraki adımı belirle
      const rawMeResponse = await fetchWithAuth(API_ENDPOINTS.CUSTOMER_ME);
      if (rawMeResponse.ok) {
          const meData = await rawMeResponse.json() as CustomerProfile;
          
          // Eğer customerIdToUse verifyData'dan gelmediyse veya boşsa, meData.id'yi kullan
          if (!customerIdToUse) {
          ...
          const storedEmail = localStorage.getItem('yabanciSaglikInitialEmail');
          ...
          if (customerIdToUse) {
            localStorage.setItem('yabanciSaglikProposalId', customerIdToUse);
          }
          ...
          if (isCoreDataMissing) {
              ...
              // Profil bilgileri tam, otomatik talep oluştur
      ...
      // onNext() is now conditional based on CUSTOMER_ME response
  ...
  const handleUpdateAdditionalInfo = async (values: FormikValues) => {
    ...
    try {
      ...
      if (customerIdForProposal) {
        localStorage.setItem('yabanciSaglikProposalId', customerIdForProposal);
      } else {
      }
  ...
  const handleCreateRequestInPersonalInfo = async (tokenToUse?: string) => {
    try {
      ...
      if (!currentCustomerId) {
        ...
        localStorage.setItem('yabanciSaglikRequestError', 'Müşteri bilgisi bulunamadı. Lütfen önceki adımları kontrol edin.');
        onNext();
        return;
      }
      ...
      if (!currentToken) {
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('yabanciSaglikRequestError', 'Kimlik doğrulama sorunu. Lütfen sayfayı yenileyin.');
        onNext();
        return;
      }
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (errorData.errors && Array.isArray(errorData.errors) && errorData.errors.length > 0) {
            ...
            if (apiError.includes('zaten açık bir yeni satış fırsatı talebi bulunmaktadır')) {
              errorMessage = 'Bu müşteri için zaten açık bir Yabancı Sağlık Sigortası talebi bulunmaktadır. Mevcut talebin sonuçlanmasını bekleyiniz.';
            } else {
        ...
        // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
        localStorage.setItem('yabanciSaglikRequestError', errorMessage);
        onNext();
        return;
      }
      ...
      localStorage.setItem('yabanciSaglikRequestSuccess', 'true');
      localStorage.removeItem('yabanciSaglikRequestError');
      
      // Otomatik talep oluşturma - analytics talep event
      pushToDataLayer({
        event: "yabancisaglik_formsubmit",
        form_name: "yabancisaglik_talep"
      });
      ...
      // Hata durumunu localStorage'a kaydet ve RequestStep'te göster
      localStorage.setItem('yabanciSaglikRequestError', error instanceof Error ? error.message : 'Talep oluşturulurken bir hata oluştu');
      onNext();
    } finally {
      setIsLoading(false);
    }
    ...
    return (
      <Box component="form" onSubmit={formik.handleSubmit} noValidate>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
          Yabancı Sağlık Sigortası teklifiniz için eksik bilgilerinizi doldurunuz
        </Typography>
            ...
            <Box sx={{ flex: 1, minWidth: '300px' }}>
              <TextField
                ...
                onChange={(e) => {
                  ...
                  formik.setFieldValue('fullName', upperCaseValue);
                  setFullNameError(null);
                }}
            ...
            <Box sx={{ flex: 1, minWidth: '250px' }}>
              <Autocomplete
                ...
                onChange={(_, newValue) => {
                  ...
                  if (newValue?.value) {
                    fetchWithAuthDistricts(newValue.value, true);
                  } else {
                    setDistricts([]);
                  }
                }}
